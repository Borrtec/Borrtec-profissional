...
let products = JSON.parse(localStorage.getItem("borrtec_products") || "null") || DEFAULT_PRODUCTS;
let cart = JSON.parse(localStorage.getItem("borrtec_cart") || "[]");
let users = JSON.parse(localStorage.getItem("borrtec_users") || "[]");
let orders = JSON.parse(localStorage.getItem("borrtec_orders") || "[]");
sessionStorage.removeItem("borrtec_currentUser");
let currentUser = JSON.parse(sessionStorage.getItem("borrtec_currentUser") || "null");
...
const save = () => {
  localStorage.setItem("borrtec_products",JSON.stringify(products));
  localStorage.setItem("borrtec_cart",JSON.stringify(cart));
  localStorage.setItem("borrtec_users",JSON.stringify(users));
  localStorage.setItem("borrtec_orders",JSON.stringify(orders));
  if(currentUser) sessionStorage.setItem("borrtec_currentUser",JSON.stringify(currentUser));
  else sessionStorage.removeItem("borrtec_currentUser");
};
...
function viewProduct(id){
  ...
  <div class="cart-layout" style="margin-top:15px">
    <div class="info-card"><div class="product-image ${p.tone||""}" style="height:420px;border-radius:12px"></div></div>
    <div class="summary">
...
function confirmCheckout(){
  ...
  if(!cleanDoc||!(cleanDoc.length===11||cleanDoc.length===14)){toast("Informe um CPF (11 dígitos) ou CNPJ (14 dígitos).");return}
  const deliveryType=el("deliveryType").value;
  let delivery={type:deliveryType};
  if(deliveryType==="delivery"){
    const cep=el("checkoutCep").value.trim(),address=el("checkoutAddress").value.trim(),number=el("checkoutNumber").value.trim(),neighborhood=el("checkoutNeighborhood").value.trim(),city=el("checkoutCity").value.trim();
    if(!cep||!address||!number||!neighborhood||!city){toast("Preencha o endereço completo para entrega.");return}
    delivery={type:"delivery",cep,address,number,complement:el("checkoutComplement").value.trim(),neighborhood,city};
  }else delivery.pickupNote="Cliente irá buscar na Borrtec";
  const items=cart.map(i=>({id:i.id,name:products.find(p=>p.id===i.id)?.name,qty:i.qty,price:products.find(p=>p.id===i.id)?.price||0}));
  const total=items.reduce((sum,i)=>sum+i.price*i.qty,0);
  const order={id:1000+orders.length+1,userId:currentUser.id,userName:currentUser.name,items,total,status:"Recebido",createdAt:new Date().toLocaleString("pt-BR"),document:documentValue,delivery,payment:el("checkoutPayment").value};
  orders.unshift(order);
  currentUser.document=documentValue;
  if(deliveryType==="delivery")Object.assign(currentUser,{cep:delivery.cep,address:delivery.address,addressNumber:delivery.number,complement:delivery.complement,neighborhood:delivery.neighborhood,city:delivery.city});
  users=users.map(u=>u.id===currentUser.id?currentUser:u);
  cart=[];save();updateCartCount();document.getElementById("checkoutModal")?.remove();toast("Pedido enviado para a Borrtec!");showPage("orders");
}

function renderOrders(){
  if(!currentUser){el("app").innerHTML=`<div class="empty"><h2>Meus pedidos</h2><p>Entre na sua conta para acompanhar seus pedidos.</p><button class="primary" onclick="openLogin()">ENTRAR</button></div>`;return}
...
function customerLogin(){
  const email=el("loginEmail").value.trim().toLowerCase(),pass=el("loginPassword").value;
  const u=users.find(x=>x.email===email&&x.password===pass);
  if(!u){toast("E-mail ou senha incorretos.");return}
  currentUser=u;save();closeModal("loginModal");toast(`Bem-vindo(a), ${u.name}!`);showPage("home");
}
function registerCustomer(){
  const name=el("regName").value.trim(),email=el("regEmail").value.trim().toLowerCase(),phone=el("regPhone").value.trim(),documentValue=el("regDocument").value.trim(),password=el("regPassword").value;
  if(!name||!email||!password||!documentValue){toast("Preencha nome, CPF/CNPJ, e-mail e senha.");return}
  const cleanDoc=documentValue.replace(/\D/g,"");
  if(cleanDoc.length!==11&&cleanDoc.length!==14){toast("CPF deve ter 11 dígitos ou CNPJ 14 dígitos.");return}
  if(users.some(u=>u.email===email)){toast("Este e-mail já está cadastrado.");return}
  const u={id:Date.now(),name,email,phone,document:documentValue,password};users.push(u);currentUser=u;save();closeModal("registerModal");toast("Conta criada com sucesso!");showPage("home");
}
function logout(){currentUser=null;save();toast("Você saiu da conta.");showPage("home")}

function adminLogin(){
  // SEGURANÇA: não existe mais senha administrativa no código do navegador.
  // O login real deverá ser validado pelo backend antes do uso em produção.
  toast("Acesso administrativo em preparação. O login seguro será conectado ao backend.");
}
...
function renderAdmin(){
  ...
  const total=orders.reduce((s,o)=>s+o.total,0),pending=orders.filter(o=>o.status!=="Entregue").length;
...
function adminOrders(){
  return `<div class="section-title"><div><h1>Pedidos</h1><p>Gerencie os pedidos recebidos.</p></div></div>${orders.length?`<div style="overflow:auto"><table class="admin-table"><thead><tr><th>Pedido</th><th>Cliente</th><th>Total</th><th>Status</th><th>Ação</th></tr></thead><tbody>${orders.map(o=>`<tr><td>#${o.id}</td><td>${escapeHtml(o.userName)}</td><td>${money(o.total)}</td><td><select onchange="updateOrderStatus(${o.id},this.value)" style="background:#101a24;color:white;border:1px solid #334151;padding:7px;border-radius:7px">${["Recebido","Em preparação","Enviado","Entregue"].map(s=>`<option ${o.status===s?"selected":""}>${s}</option>`).join("")}</select></td><td><button class="small-btn yellow" onclick="adminViewOrder(${o.id})">Ver</button></td></tr>`).join("")}</tbody></table></div>`:`<div class="empty">Nenhum pedido recebido ainda.</div>`}`;
}
function updateOrderStatus(id,status){const o=orders.find(x=>x.id===id);if(o){o.status=status;save();toast(`Pedido #${id} atualizado.`);renderAdmin()}}
function adminViewOrder(id){
  const o=orders.find(x=>x.id===id);if(!o)return;
  alert(`Pedido #${o.id}\nCliente: ${o.userName}\nCPF/CNPJ: ${o.document||"Não informado"}\nRecebimento: ${o.delivery?.type==="pickup"?"Buscar na Borrtec":`${o.delivery?.address||""}, ${o.delivery?.number||""} - ${o.delivery?.neighborhood||""}, ${o.delivery?.city||""} CEP ${o.delivery?.cep||""}` }\nPagamento: ${o.payment||"Não informado"}\n\n${o.items.map(i=>`${i.qty}x ${i.name} — ${money(i.price*i.qty)}`).join("\n")}\n\nTotal: ${money(o.total)}\nStatus: ${o.status}`);
}
function adminProducts(){
  return `<div class="section-title"><div><h1>Produtos</h1><p>Adicione, edite e acompanhe seu estoque.</p></div><button class="primary" onclick="newProduct()">+ NOVO PRODUTO</button></div>
  <div style="overflow:auto"><table class="admin-table"><thead><tr><th>Produto</th><th>Categoria</th><th>Preço</th><th>Estoque</th><th>Ações</th></tr></thead><tbody>${products.map(p=>`<tr><td>${escapeHtml(p.name)}</td><td>${escapeHtml(p.category)}</td><td>${money(p.price)}</td><td>${p.stock}</td><td><div class="admin-actions"><button class="small-btn yellow" onclick="editProduct(${p.id})">Editar</button><button class="small-btn danger" onclick="deleteProduct(${p.id})">Excluir</button></div></td></tr>`).join("")}</tbody></table></div>`;
}
function adminCustomers(){
  return `<div class="section-title"><div><h1>Clientes</h1><p>Clientes cadastrados no sistema.</p></div></div><div style="overflow:auto"><table class="admin-table"><thead><tr><th>Nome</th><th>E-mail</th><th>Telefone</th><th>Pedidos</th></tr></thead><tbody>${users.map(u=>`<tr><td>${escapeHtml(u.name)}</td><td>${escapeHtml(u.email)}</td><td>${escapeHtml(u.phone||"-")}</td><td>${orders.filter(o=>o.userId===u.id).length}</td></tr>`).join("")}</tbody></table></div>`;
}
function newProduct(){editingProductId=null;openProductForm()}
function editProduct(id){editingProductId=id;openProductForm()}
function openProductForm(){
  const p=products.find(x=>x.id===editingProductId)||{name:"",category:"Borracha",price:"",stock:"",desc:"",tone:"black"};
  const modal=document.createElement("div");modal.className="modal open";modal.id="productFormModal";
  modal.innerHTML=`<div class="modal-card"><button class="close" onclick="document.getElementById('productFormModal').remove()">×</button><div class="modal-title"><span class="yellow-icon">▣</span><div><h2>${editingProductId?"Editar produto":"Novo produto"}</h2><p>Cadastre as informações da peça.</p></div></div><div class="product-form"><label>Nome<input id="pfName" value="${escapeAttr(p.name)}"></label><label>Categoria<select id="pfCat"><option ${p.category==="Borracha"?"selected":""}>Borracha</option><option ${p.category==="Silicone"?"selected":""}>Silicone</option><option ${p.category==="Peças"?"selected":""}>Peças</option></select></label><label>Preço<input id="pfPrice" type="number" step="0.01" value="${p.price}"></label><label>Estoque<input id="pfStock" type="number" value="${p.stock}"></label><label>Visual<select id="pfTone"><option value="black" ${p.tone==="black"?"selected":""}>Preto</option><option value="blue" ${p.tone==="blue"?"selected":""}>Azul</option><option value="red" ${p.tone==="red"?"selected":""}>Vermelho</option><option value="bar" ${p.tone==="bar"?"selected":""}>Perfil</option></select></label><label>Descrição<input id="pfDesc" value="${escapeAttr(p.desc)}"></label><button class="primary full-row" onclick="saveProductForm()">SALVAR PRODUTO</button></div></div>`;
  document.body.appendChild(modal);
}
function saveProductForm(){
  const name=el("pfName").value.trim(),price=Number(el("pfPrice").value),stock=Number(el("pfStock").value),desc=el("pfDesc").value.trim();
  if(!name||!price){toast("Informe nome e preço.");return}
  const data={name,category:el("pfCat").value,price,stock,desc,tone:el("pfTone").value};
  if(editingProductId){Object.assign(products.find(p=>p.id===editingProductId),data);toast("Produto atualizado!")}
  else{products.push({id:Date.now(),...data});toast("Produto adicionado!")}
  save();document.getElementById("productFormModal").remove();renderAdmin();
}
function deleteProduct(id){if(!confirm("Excluir este produto?"))return;products=products.filter(p=>p.id!==id);cart=cart.filter(i=>products.some(p=>p.id===i.id));save();toast("Produto excluído.");renderAdmin()}
